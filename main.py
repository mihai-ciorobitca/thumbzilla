...
async def sign_up(page, username, password):
    await page.goto("https://www.thumbzilla.com", wait_until="networkidle")

    try:
        age_button = page.locator("#modalWrapMTubes > div > div > button")
        await age_button.click()
    except:
        print("Age confirmation button not found or already handled.")

    await page.wait_for_selector("#cookieBannerWrapper", timeout=10000)
    ...
    await page.evaluate("""
        const btn = document.querySelector('.cbPrimaryCTA ')
        if (btn) {
            btn.click()
        } else {
    ...
    # signup_btn = page.locator("#signup-btn")
